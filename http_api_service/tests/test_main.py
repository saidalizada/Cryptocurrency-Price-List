...
def test_get_crypto_prices():
    ...
    response = client.get("/crypto-price-and-rank/")
    assert response.status_code == 200
    assert "prices" in response.json()
    assert len(response.json()["prices"]) == 10

    # Test with a custom limit (5)
    response_custom_limit = client.get("/crypto-price-and-rank/?limit=5")
    ...
    response_invalid_limit = client.get("/crypto-price-and-rank/?limit=-1")
